...
class BehavTrial(Trial):
    ...
    def __init__(self, session, trial_nr, phase_durations, parameters=None, timing='seconds', 
        ...
        print(f"This is the ball radius: {self.settings['ball']['radius']}")
        
        # Create ball stimulus
        # self.ball = visual.Circle(
        #     win=session.win,
        #     radius=session.settings['ball']['radius'],
        #     fillColor=np.clip(utils.oklab_to_rgb([self.ball_start_color, 0, 0], psychopy_rgb=True), -1, 1),
        #     lineWidth=0,
        #     pos=(0, 0)
        # )
    ...
    #     if self.verbose:
    #         print(f"Phase 0: Fixation Display ({self.phase_durations[0]}s)")
    #     self.phase = 0
    #     self.display_fixation()
    #     self.phase_clock.reset()
    #     self.wait_until_phase_ends()
        
    #     # Phase 1: Interactor Display
    #     if self.verbose:
    #         print(f"Phase 1: Interactor Display ({self.phase_durations[1]}s)")
    #     self.phase = 1
    #     self.display_interactor()
    #     self.phase_clock.reset()
    #     self.wait_until_phase_ends()
        
    #     # Phase 2: Occluder Display
    #     if self.verbose:
    #         print(f"Phase 2: Occluder Display ({self.phase_durations[2]}s)")
    #     self.phase = 2
    #     self.display_occluder()
    #     self.phase_clock.reset()
    #     self.wait_until_phase_ends()
        
    #     # Phase 3: Ball Movement
    #     if self.verbose:
    #         print(f"Phase 3: Ball Movement ({self.phase_durations[3]}s)")
    #     self.phase = 3
    #     self.run_ball_movement()
    ...
    def draw(self):
        ...
        if self.phase == 0:
            ...
            # Phase 3: Ball Movement
            # Handle ball movement directly in the draw method
        ...
        # No need to call win.flip() here, the parent class's run method will handle this

    def handle_ball_movement(self):
        """Handle the ball movement during phase 3."""
        # This method will be called repeatedly during phase 3
        ...
        if not self.responded and not self.exit_phase:
            ...
            # Check for key presses (moved to get_events in parent class)
    ...
    def display_interactor(self):
        """Display interactor elements."""
        self.draw_common_elements()
        self.draw_interactor_elements()
        self.session.win.flip()

    def display_occluder(self):
        """Display occluder."""
        self.draw_common_elements()
        self.draw_interactor_elements()
        self.session.occluder.draw()
    ...
    def run_ball_movement(self):
        """Run the ball movement phase."""
        self.phase_clock.reset()
        frame_dur = self.session.settings['display']['frame_dur']
        trial_duration = self.phase_durations[3]
        decay_constant = self.session.settings['ball']['decay_constant']
        occluder_radius = self.session.settings['occluder']['radius']
        ball_radius = self.session.settings['ball']['radius']
        
        while self.phase_clock.getTime() < trial_duration:
            # Apply ball speed decay
            decay_factor = physics.calculate_decay_factor(
                self.ball_speed, self.ballmov_time, trial_duration, constant=decay_constant
            )
            self.velocity = [self.velocity[0] * decay_factor, self.velocity[1] * decay_factor]
            self.ball.pos += tuple([self.velocity[0], self.velocity[1]])
            ...
            # Track when ball enters and leaves screen
            square_size = self.session.settings['display']['square_size']
            if (np.linalg.norm(self.ball.pos) > square_size / 2) and self.phase_clock.getTime() < (trial_duration // 2):
                self.enter_screen_time = self.phase_clock.getTime()
                self.entered_screen = True
            ...
            if (self.entered_screen and 
                self.enter_screen_time is not None and 
                self.left_screen_time is None and
                self.phase_clock.getTime() > (trial_duration // 2) and
                (np.linalg.norm(self.ball.pos) > square_size // 2)):
                self.left_screen_time = self.phase_clock.getTime()
                if self.verbose:
                    screen_time = self.left_screen_time - self.enter_screen_time
            ...
            self.session.occluder.draw()
            self.session.win.flip()
            core.wait(frame_dur)  # Control frame rate
            ...
            if keys and not self.responded:
                if self.verbose:
                    print(f"Response: {keys[0]}")
                
                if not self.left_occluder:
                    print(f"Wrong, too early")
                    self.session.exp_data["response"][-1] = keys[0]
                    self.correct_response = None
                    self.responded = True
                else:
                    toets_moment = self.phase_clock.getTime()
                    ...
                    if self.ball_change and keys[0] in ["x", "m"]:
                        if keys[0] == self.session.button_order["brighter"]:
                            this_response = "brighter"
                        elif keys[0] == self.session.button_order["darker"]:
                            this_response = "darker"
                    ...
                    self.responded = True
            elif self.phase_clock.getTime() > trial_duration and not self.responded:
                if self.ball_change:
                    self.correct_response = False
                    if self.verbose:
                        print(f"Undetected ball change, there was a hue change of the {ball_direction}ward ball")
                else:
                    self.correct_response = None
    ...
    def draw_interactor_elements(self):
        ...
        if self.trial_option[:4] != "none":
            interactor = stimuli.create_interactor(
                ...
                path_45=session.settings['interactor']['path_45'],
                path_135=session.settings['interactor']['path_135'],
            )
            interactor.draw()
...
class BehavSession(Session):
    ...
    def setup_experiment_elements(self):
        ...
        win_dims = self.settings['display']['win_dims']
    ...
    def draw_common_elements(self):
        """Draw common screen elements."""
        for border in self.screen_borders:
            border.draw()
        
        for fix_line in self.cross_fix:
            fix_line.draw()
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--settings', type=str, default='config_lumin.yaml', help='Settings file')
    ...
    session.create_trials(durations=(6, 2, 8, 10), 
                        timing='seconds')
...
########
    ...
    # session.create_trials(durations=(.25, .25), 
    #                       timing='seconds')
